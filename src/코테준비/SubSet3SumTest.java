...
//		subSet2(0, 0, "");
	...
	public static void subSet3(int index, int total) {
		if(total == sum) {
			ea++;
			return;
		}
		else if(total > sum || index == arr.length) {
			return;
		}
		subSet3(index+1, total+arr[index]);
		subSet3(index+1, total);
	}
}
