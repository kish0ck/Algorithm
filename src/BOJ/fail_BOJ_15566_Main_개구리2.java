...
public class fail_BOJ_15566_Main_개구리2 {
	...
	public static void main(String[] args) throws IOException {
		...
		boolean[] lists = new boolean[N];
		for (int i = 0; i < N; i++) {
			if(frog_love[i].size()==1) {
				list.add(i);
				cnt++;
				lists[i]=true;
			} 
...
loop:	for (int i = 0; i < N; i++) {
			if(i<cnt) {
				int frog_num = list.get(i);
				visited[frog_num]=true;
				lotus[frog_love[frog_num].get(0)] = frog_num;
				dfs(0);
				if(flag) break;
				visited[frog_num]=false;
				lotus[frog_love[frog_num].get(0)] = -1;
			}else {
				int frog_num = list.get(i);
				for (int j = 0; j < 2; j++) {
					lotus[frog_love[frog_num].get(j)] = frog_num;
					visited[frog_num]=true;
					dfs(0);
					if(flag) break loop;
					visited[frog_num]=false;
					lotus[frog_love[frog_num].get(j)] = -1;
				}
			}
		}
